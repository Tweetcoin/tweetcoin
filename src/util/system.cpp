...
const char * const BITCOIN_CONF_FILENAME = "tweetcoin.conf";
...
#else
    const char* pszModule = "tweetcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tweetcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tweetcoin";
#else
    // Unix
    return pathRet / ".tweetcoin";
#endif
#endif
}
