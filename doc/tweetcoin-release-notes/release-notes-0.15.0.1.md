Tweetcoin Core version *0.15.0* is now available from:

  <https://download.tweetcoin.us/tweetcoin-0.15.0/>
...
installer (on Windows) or just copy over `/Applications/Tweetcoin-Qt` (on Mac)
or `tweetcoind`/`tweetcoin-qt` (on Linux).
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Tweetcoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Tweetcoin Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Tweetcoin, either on the command line or in the Tweetcoin config file.
...
Tweetcoin Core 0.15.0 contains the following changes to the RPC interface and `tweetcoin-cli` for multi-wallet:

* When running Tweetcoin Core with a single wallet, there are **no** changes to the RPC interface or `tweetcoin-cli`. All RPC calls and `tweetcoin-cli` commands continue to work as before.
* When running Tweetcoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `tweetcoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Tweetcoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:2223/wallet/wallet1.dat/`. `tweetcoin-cli` commands should be run with a `-rpcwallet` option, for example `tweetcoin-cli -rpcwallet=wallet1.dat getbalance`.
...
- When using Tweetcoin Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- #2222 `fa625b0` Document CWalletTx::mapValue entries and remove erase of nonexistent "version" entry (ryanofsky)
